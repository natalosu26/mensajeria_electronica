...
from flask import (
    Blueprint, g, redirect, request, url_for, flash, render_template, session
)
...
def register():
    try:
        ...
        if request.method == 'POST':
            ...
            content = 'Hello there, to activate your account, please click on this link ' + url_for('auth.activate', _external=True) + '?auth=' + number
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            if user is not None:
                ...
                content = 'Hello there, to change your password, please click on this link ' + url_for('auth.change', _external=True) + '?auth=' + number
...
def send_email(credentials, receiver, subject, message):
    ...
    # Send Email
    smtp = SMTP("smtp-mail.outlook.com", port=587)
    smtp.starttls()
